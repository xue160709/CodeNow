...
- 只执行完成本次提交所需的只读 Git 检查、精确暂存和 `git commit`；不要修改业务文件来“凑”一次提交。
- Codex Skill 不使用 `allowed-tools` 作为权限边界；实际命令仍受当前会话沙箱、审批策略和仓库权限约束。
- 暂存前先展示或核对目标文件，避免把无关改动、密钥或用户未授权的内容带入提交。
